...
export class GitHubProvider implements DocumentationProvider {
  ...
  async getAllFiles(repoUrl: string, callbacks?: ProgressCallback): Promise<FileContent[]> {
    const repoInfo = GitHubProvider.extractRepoInfo(repoUrl);
    if (!repoInfo) {
    ...
    try {
      ...
      const response = await GitHubProvider.fetchWithRetry(treeUrl);
      ...
      for (let i = 0; i < docFiles.length; i++) {
        ...
        try {
          ...
          const contentResponse = await GitHubProvider.fetchWithRetry(rawBaseUrl + file.path);
